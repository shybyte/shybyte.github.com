...
Filters.convolute=function(a,d,b){for(var c=Math.round(Math.sqrt(d.length)),e=Math.floor(c/2),f=a.data,j=a.width,a=a.height,h=Filters.createImageData(j,a),g=h.data,b=b?1:0,k=0;k<a;k++)for(var i=0;i<j;i++){for(var m=k,q=i,n=4*(k*j+i),u=0,r=0,v=0,s=0,w=0;w<c;w++)for(var x=0;x<c;x++){var l=m+w-e,t=q+x-e;0<=l&&l<a&&0<=t&&t<j&&(l=4*(l*j+t),t=d[w*c+x],u+=f[l]*t,r+=f[l+1]*t,v+=f[l+2]*t,s+=f[l+3]*t)}g[n]=u;g[n+1]=r;g[n+2]=v;g[n+3]=s+b*(255-s)}return h};
Filters.fastGaussian=function(a){for(var d=[0.06,0.061,0.242,0.383,0.242,0.061,0.06],b=a.data,c=a.width,a=a.height,e=Filters.createImageData(c,a).data,f=e,j=0;j<a;j++)for(var h=0;h<c;h++){for(var g=0,k=0,i=0,m=4*(j*c+h-3),q=0;q<d.length;q++){var n=d[q],g=g+b[m++]*n,k=k+b[m++]*n,i=i+b[m++]*n;m++}m=4*(j*c+h);f[m++]=g;f[m++]=k;f[m++]=i;f[m]=255}b=e;e=Filters.createImageData(c,a);f=e.data;for(j=0;j<a;j++)for(h=0;h<c;h++){i=k=g=0;m=4*((j-3)*c+h);for(q=0;q<d.length;q++)var n=d[q]/1.109,u=m+4*q*c,g=g+b[u++]*
n,k=k+b[u++]*n,i=i+b[u]*n;m=4*(j*c+h);f[m++]=g;f[m++]=k;f[m++]=i;f[m]=255}return e};Filters.compressedGrayScaleFromRed=function(a){for(var d=a.data,b=[],c=a.width*a.height,e=0,f=0;f<c;f++)b[f]=d[e],e+=4;return{data:b,width:a.width,height:a.height}};Filters.imageDataFromCompressedGrayScale=function(a){for(var d=Filters.createImageData(a.width,a.height),b=a.data,a=a.width*a.height,c=d.data,e=0,f=0;f<a;f++)c[e]=c[e+1]=c[e+2]=b[f],c[e+3]=255,e+=4;return d};
Filters.meanCGSRepeated=function(a,d,b){for(var c=0;c<b;c++)a=Filters.meanCGS(a,d);return a};Filters.meanCGS=function(a,d){return Filters.meanCGSHorizontal(Filters.meanCGSVertical(a,d),d)};Filters.meanCGSHorizontal=function(a,d){var b=a.data,c=a.width*a.height,e=[],f=0,j=0;initArray(e,c);for(var h=Math.floor(d/2),g=d;g<c;g++)f=f+b[g]-b[j++],e[g-h+1]=f/d;return{data:e,width:a.width,height:a.height}};
Filters.meanCGSVertical=function(a,d){var b=a.data,c=a.width,e=a.height,f=c*e,j=[];initArray(j,f);for(var h=Math.floor(d/2),g=0;g<c;g++)for(var k=0,i=g-h*c,m=g+(h+1)*c,q=g,n=0;n<e;n++)k=k+(m<f?b[m]:0)-(0<=i?b[i]:0),j[q+c]=k/d,i+=c,m+=c,q+=c;return{data:j,width:a.width,height:a.height}};
Filters.peaksCGSOld=function(a,d){var b=a.data,c=a.width,e=a.height,f=[];initArray(f,c*e);for(var j=0;j<c;j++)for(var h=0;h<e;h++){var g=h*c+j,k=b[g];k>d&&k>=b[g+1]&&k>=b[g-1]&&k>=b[g-c]&&k>=b[g+c]&&(f[g]=255)}return{data:f,width:a.width,height:a.height}};
Filters.peaksCGS=function(a,d,b){var c=a.data,e=a.width,f=a.height,j=[],h=[];initArray(h,e*f);for(var g=0;g<e;g++)for(var k=0;k<f;k++){var i=k*e+g,m=c[i];if(m>=d){h[i]=255;var q=Math.min(g+b,e),n=Math.max(g-b,0);a:for(;n<q;n++)for(var u=Math.min(k+b,f),r=Math.max(k-b,0);r<u;r++)if(m<c[r*e+n]&&(g!=n||k!=r)){h[i]=0;break a}255==h[i]&&j.push({x:g,y:k})}}return{data:h,width:a.width,height:a.height,peaks:j}};
Filters.thresholdCGS=function(a,d){for(var b=a.data,c=a.width,e=a.height,f=[],j=0;j<c;j++)for(var h=0;h<e;h++){var g=h*c+j;f[g]=b[g]>=d?255:0}return{data:f,width:a.width,height:a.height}};function initArray(a,d){for(var b=0;b<d;b++)a[b]=0};(function(){var a,d,b,c,e,f,j,h,g,k,i=Array.prototype.indexOf||function(a){for(var b=0,c=this.length;b<c;b++)if(this[b]===a)return b;return-1};g=0;c=null;a=["0","1","2","3","4"];e=["0","1"];k="marking";b=function(){function a(b,d){var e;this.pos=b;this.type=d;e=this;this.id=g++;this.el=jq("<div/>").addClass("marking markingType"+this.type).attr({id:this.id,draggable:!1}).bind("dragend",function(){return log("dragend")}).bind("dragstart",function(){e.el.css("opacity","0.4");c=e;return log("dragstart")});
...
a.width()*l.width)+p.x,y:Math.round(b.y/a.height()*l.height)+p.y}};t=function(){var a,b;a=f.val();a/=128;1>a?b=1:(b=2-a,a=1);jq("#mainCanvas").css("opacity",b);return jq("#filteredCanvas").css("opacity",a)};x=function(a){w(a,H());return z()};H=function(){return jq("input:radio[name=markingType]:checked").val()};w=function(c,d){var e;e=new b(c,d);o.push(e);g.append(e.el);e.updateScreenPos(l,a,p);return A()};Q=function(a){if(a=V(a.x,a.y))return o=_.without(o,a),a.el.remove(),A(),z()};J=function(){var a,
b,c;for(b=0,c=o.length;b<c;b++)a=o[b],a.el.remove();o=[];return A()};A=function(){var a,b,c,d,f,g;b=_.groupBy(o,"type");a=function(a){var c,d;return null!=(c=null!=(d=b[a])?d.length:void 0)?c:0};g=[];for(d=0,f=e.length;d<f;d++)c=e[d],g.push(jq("#cellCount"+c).text(a(c)));return g};V=function(a,b){return _.min(o,function(c){var d;d=c.pos.x-a;c=c.pos.y-b;return d*d+c*c})};N=function(a){var b;b=new FileReader;b.onload=function(b){M(b.target.result);return C=a.name};return b.readAsDataURL(a)};M=function(a){var b;
r.hide("slow");b=new Image;b.onload=function(){return P(b)};return b.src=a};P=function(a){p={x:0,y:0,width:a.width,height:a.height};K=a;l.width=a.width;l.height=a.height;y.drawImage(a,0,0);W();O();return F()};F=function(){var a;G.width=l.width;G.height=l.height;a=Filters.filterCanvas(Filters.thresholdRG,l,{threshold:s.val()});return U.putImageData(a,0,0)};window.FileReader||(alert("No local file reading possible. "+d),jq("#openFile").replaceWith("No local file reading possible. "+a.html()));(function(){var a;
if(a=localStorage.cell_counter_settings)a=JSON.parse(a),e=a.enabledMarkingTypes||["0","1"],s.val(a.threshold),h.val(a.markingsSize),f.val(a.fadeThresholdImage),I(),t();return B()})();j(B);$("#helpLink").overlay({mask:{color:"#ebecff",loadSpeed:200,opacity:0.7},closeOnClick:!1});B();(function(){var a;a=jq("#openFile");return a.change(function(){var b;b=a.get(0).files;return N(b[0])})})();(function(){return g.bind("dragover",function(){return!1}).bind("dragleave",function(){l.className="";return!1}).bind("drop",
function(a){l.className="";a.preventDefault();if(0<a.originalEvent.dataTransfer.files.length)return N(a.originalEvent.dataTransfer.files[0]);if(c)return 1})})();(function(){g.click(function(a){var b;if("marking"===k)return b=E(a),a.ctrlKey&&0<o.length?Q(b):x(b)});return g.bind("contextmenu",function(a){a.preventDefault();return Q(E(a))})})();(function(){return $("#autoCountButton").click(function(){J();$("#countingMessage").show();return setTimeout(function(){var a,b,c,d,e;a=Filters.compressedGrayScaleFromRed(y.getImageData(0,
0,l.width,l.height));a=Filters.meanCGSRepeated(a,4,4);a=Filters.peaksCGS(a,s.val(),3);b=H();e=a.peaks;for(c=0,d=e.length;c<d;c++)a=e[c],w({x:a.x+p.x,y:a.y+p.y},b);z();return $("#countingMessage").hide("slow")},19)})})();(function(){var b,c,d,e,f,h,j,i;b=$("#canvasAndHelpContainer");O();d=$("#helpText");c=$("#cropSelection");h=i=null;$("#cropImageLink").click(function(){i=[];k="crop";d.text("Click the top left point! Press the ESC key to cancel cropping.");d.show("slow");return b.expose({color:"#333",
onClose:function(){d.hide();c.hide("slow");return k="marking"}})});g.mousemove(function(a){var b;if("crop"===k&&1===i.length)return b=D(a),a=b.x-h.x,b=b.y-h.y,c.css({width:a+"px",height:b+"px"})});g.click(function(a){var b;if("crop"===k){b=D(a);c.css({top:b.y,left:b.x,width:"5px",height:"5px"}).show();h=b;i.push(E(a));if(1===i.length)return d.text("Click the bottom right point! Press the ESC key to cancel cropping.");if(1<i.length)return $.mask.close(),j(),e({x:i[0].x,y:i[0].y,width:i[1].x-i[0].x,
height:i[1].y-i[0].y})}});j=function(){var a;if(i[1].x<i[0].x)a=i[0].x,i[0].x=i[1].x,i[1].x=a;if(i[1].y<i[0].y)return a=i[0].y,i[0].y=i[1].y,i[1].y=a};e=function(a){var b;b=y.getImageData(a.x-p.x,a.y-p.y,a.width,a.height);p=a;l.width=a.width;l.height=a.height;y.putImageData(b,0,0);F();f();r.show("slow");return R("cropWindow",p)};f=function(){var b,c,d,e,f,g;for(d=0,e=o.length;d<e;d++)b=o[d],c=b.pos,p.x<=(f=c.x)&&f<p.x+l.width&&p.y<=(g=c.y)&&g<p.y+l.height?b.updateScreenPos(l,a,p):(b.el.remove(),b.removed=
!0);e=[];for(c=0,d=o.length;c<d;c++)b=o[c],b.removed||e.push(b);o=e;z();A();return v.hide("slow")};r.click(function(){r.hide("slow");return P(K)});return v.click(function(){return e(T)})})();(function(){var a,b;a=function(a,b){return a.hide().rangeinput().change(function(){S();return b()})};b=function(b,c){return a(b,c).bind("onSlide",c)};h=b(h,I);s=a(s,F);return f=b(f,t)})();M("images/nora1.jpg");(function(){return jq(window).resize(function(){var b,c,d,e;e=[];for(c=0,d=o.length;c<d;c++)b=o[c],e.push(b.updateScreenPos(l,
a,p));return e})})();$("#removeAllMarkings").click(function(){J();return z()});return $("#filterButton").click(function(){var a;a=Filters.filterCanvas(Filters.fastGaussian,l,[0,-1,0,-1,5,-1,0,-1,0]);return y.putImageData(a,0,0)})};j=function(b){var c;c=$("#enabledMarkingTypesSelector");$("#configureLink").click(function(){var b,d,f,g,h;c.empty();h=[];for(f=0,g=a.length;f<g;f++)d=a[f],b=0<=i.call(e,d)?"checked":"",b='<div>\n<input id="enableMarkingType'+d+'" type="checkbox" value="'+d+'" '+b+'>\n<label for="enableMarkingType'+
d+'" class="markingLabel'+d+'">Cell Type '+d+"</label>\n                             </div>",h.push(c.append(b));return h}).overlay({mask:{color:"#ebecff",loadSpeed:200,opacity:0.7},closeOnClick:!1});return jq("#configureOKButton").click(function(){e=[];$("input:checked",c).each(function(a,b){return e.push($(b).val())});return b()})};d="Your browser is too old. Please use a newer version of firefox, google chrome or opera.";h=function(){return window.applicationCache.addEventListener("updateready",
function(){if(window.applicationCache.status===window.applicationCache.UPDATEREADY&&(window.applicationCache.swapCache(),confirm('A new version of "Marcos Cell Counter" is available. Load it now?')))return window.location.reload()},!1)};jq(function(){if(isCanvasSupported())return h(),f();jq("#openFile").hide();return alert(d)})}).call(this);
