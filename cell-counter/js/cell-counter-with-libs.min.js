...
return $("#filterButton").click(function(){var a;a=Filters.filterCanvas(Filters.fastGaussian,l,[0,-1,0,-1,5,-1,0,-1,0]);return y.putImageData(a,0,0)})};j=function(b){var c;c=$("#enabledMarkingTypesSelector");$("#configureLink").click(function(){var b,d,f,g,h;c.empty();h=[];for(f=0,g=a.length;f<g;f++)d=a[f],b=0<=i.call(e,d)?"checked":"",b='<div>\n<input id="enableMarkingType'+d+'" type="checkbox" value="'+d+'" '+b+'>\n<label for="enableMarkingType'+d+'" class="markingLabel'+d+'">Cell Type '+d+"</label>\n                             </div>",
h.push(c.append(b));return h}).overlay({mask:{color:"#ebecff",loadSpeed:200,opacity:0.7},closeOnClick:!1});return jq("#configureOKButton").click(function(){e=[];$("input:checked",c).each(function(a,b){return e.push($(b).val())});return b()})};d="Your browser is too old. Please use a newer version of firefox, google chrome or opera.";h=function(){return window.applicationCache.addEventListener("updateready",function(){if(window.applicationCache.status===window.applicationCache.UPDATEREADY&&(window.applicationCache.swapCache(),
confirm('A new version of "Marcos Cell Counter" is available. Load it now?')))return window.location.reload()},!1)};jq(function(){if(isCanvasSupported())return h(),f();jq("#openFile").hide();return alert(d)})}).call(this);
